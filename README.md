...
JungleGym is an open-source playground for testing and developing autonomous web agents. To be clear, it's not an Agent, but rather a tool to build agents. Here, you can download demonstration data and test your agents with ground-truth trajectories/results (using the JungleGym APIs). These Datasets are all available in the JungleGym API and in the [JungleGym](https://junglegym.ai) playground, including 6 realistic, fully functional, sandboxed websites (from WebArena) to test your Agents.
